...
➢ `useState`는 배열을 `return` 하는 `useState`함수의 값을 구조분해 할당을 사용하여 <U>getter</U>인 `state`, setter인 `setState`를 값을 사용하는 방식이다.
<br />

`state`값이 바뀌면 리렌더링 된다는 건 알겠는데..🧐 왜 `setState`로 값을 바꾸자마자 바꾼 값을 `state`로 출력할 수 없을까?  
-> 미리 말하자면 setState를 작동시킨 후 state와 setState는 서로 다른 클로저를 참조한다. setState가 참조하고 있는 건 이미 업데이트 된 클로저를 참조하고 state가 참조하고 있는 클로저는 setState를 실행시키기 전의 클로저 렌더링 되고난 후의 클로저를 참조하고 있다.
