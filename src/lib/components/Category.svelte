...
{#if $isOpenMenu}
	{#if $displaySize !== "desktop"}
		<div
			class="fixed left-0 top-0 z-50 h-screen w-screen bg-pointColor-800 opacity-10"
			on:click={isOpenMenu.toggle}
			on:keydown={isOpenMenu.toggle}
		/>
	{/if}
	<aside
		class="prose-sm fixed bottom-0 right-0 top-0 z-50 flex h-full w-[300px] flex-col justify-between overflow-y-auto border-l-[1px] bg-pointColor-500 pt-0"
	>
