...
// 				const allTags = matterResult.data.tag.split(', ').sort();

// 				allTags.forEach((item) => {
// 					tagObj[item] = tagObj[item] + 1 || 1;
// 				});
// 			}
...
// 				id,
// 				timestamp,
// 				password: matterResult.data.password || null,
// 				tags: tagObj,
// 				tagObj,
...
// 			tags.forEach((tag) => tagList.set(tag, !tagList.get(tag) ? 1 : tagList.get(tag) + 1));
// 		}
// 	});
...
// 				let param = [file.name.endsWith('.md') ? file.name.replace(/\.md$/, '') : file.name];
// 				let id = file.name;
